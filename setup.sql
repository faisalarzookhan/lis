...
║  - Additional features (analytics, notifications, file uploads, etc.)     ║
...
║                                                                            ║
║  Version: 1.0.0 - Unified and optimized for Supabase                       ║
╚══════════════════════════════════════════════════════════════════════════════╝
*/
...
-- =============================================================================
-- STEP 2: CLEANUP - Drop all existing tables (DESTRUCTIVE OPERATION)
-- =============================================================================
...
BEGIN
    ...
    FOR table_name IN
        ...
        AND tablename IN (
            'analytics_events', 'notifications', 'email_logs', 'file_uploads',
            'knowledge_base', 'chat_feedback', 'chat_messages', 'chat_sessions',
            'faqs', 'leads', 'pages', 'projects', 'testimonials', 'team_members',
            'services', 'profiles'
        )
    LOOP
...
CREATE TABLE public.team_members (
    ...
    image TEXT,
    email TEXT CHECK (email ~* '^[A-Za-z0._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'),
    linkedin_url TEXT,
    github_url TEXT,
    twitter_url TEXT,
    is_active BOOLEAN DEFAULT TRUE NOT NULL,
...
-- Chat policies for users (example: users can only access their own chat data)
